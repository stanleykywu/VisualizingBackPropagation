...
class Net {
  ...
  public double stochasticBackPropFunc(double[][] inputs, double[][] output,
    ...
    if (!this.correctClassification(outB, output)) {
      ...
      double deltaOut00 =
          2 * (outB[0][0] - output[0][0]) * activationDer.apply(inB[0][0]) * outA[0][0];
      double deltaOut01 =
          2 * (outB[0][0] - output[0][0]) * activationDer.apply(inB[0][0]) * outA[1][0];
